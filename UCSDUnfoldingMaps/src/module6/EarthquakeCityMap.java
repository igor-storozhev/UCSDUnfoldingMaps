...
public class EarthquakeCityMap extends PApplet {
	...
	private List<EarthquakeMarker> selectedCityMarkers;
	...
	private void sortAndPrint(int numToPrint) {
		// Create array of earthquake from list
		EarthquakeMarker[] arrEarthquake = quakeMarkers.toArray(new EarthquakeMarker[quakeMarkers.size()]);
		// Back to list with required type for sort
		List<EarthquakeMarker> listEarthquake = Arrays.asList(arrEarthquake);
		// sort at last
		Collections.sort(listEarthquake);
		// print top numToPrint
		Integer printed = 0;
		for (EarthquakeMarker quake : listEarthquake) {
			if (printed == numToPrint) {
				break; // stop printing
			} else {
				printed++;
			}
	...
	private void checkCitiesForClick() {
		...
		for (Marker marker : cityMarkers) {
			if (!marker.isHidden() && marker.isInside(map, mouseX, mouseY)) {
				...
				selectedCityMarkers = new ArrayList<EarthquakeMarker>(); // array
																			// for
																			// data
																			// to
																			// analyze
	...
	// TODO Helper method that will analyze data about earthquakes near clicked
	// city stored in selectedCityMarkers.
	// Results saved into lastClicked properties with enum QTOTAL, QAVG, QLAST
	...
	private void cityQuakeAnalyze() {
		...
		for (Marker qmarker : this.selectedCityMarkers) {
			...
			if ("Past Hour".equals(age) || "Past Day".equals(age)) {
				sb.append(((EarthquakeMarker) qmarker).getStringProperty("title") + "\n");
			}
		}
		// calculate avg magnitude if data exists
		if (qTotal > 0) {
			Float qAvg = qSumMagnitude / qTotal;
			// save analysis data to lastClicked properties
			this.lastClicked.setProperty(CityProperty.QTOTAL.name(), qTotal.toString());
			this.lastClicked.setProperty(CityProperty.QAVG.name(), qAvg.toString());
			this.lastClicked.setProperty(CityProperty.QLAST.name(), sb.toString());
			System.out.println("Total quakes: " + lastClicked.getProperty(CityProperty.QTOTAL.name()));
			System.out.println("Magnitude average: " + lastClicked.getProperty(CityProperty.QAVG.name()));
			System.out.println("Last quakes: " + lastClicked.getProperty(CityProperty.QLAST.name()));
		}
	}
	...
	private void cityInfo() {
		...
		if (lastClicked.getProperty(CityProperty.QTOTAL.name()) == null) {
			return; // nothing to display
		}
		/*
		 * System.out.println("Total quakes: " +
		 * lastClicked.getProperty(CityProperty.QTOTAL.name()));
		 * System.out.println("Magnitude average: " +
		 * lastClicked.getProperty(CityProperty.QAVG.name()));
		 * System.out.println("Last quakes: " +
		 * lastClicked.getProperty(CityProperty.QLAST.name()));
		 */
		String cityName = ((CityMarker) lastClicked).getCityName();
		String qTotal = this.lastClicked.getProperty(CityProperty.QTOTAL.name()).toString();
		String avgMagnitude = this.lastClicked.getProperty(CityProperty.QAVG.name()).toString();
		String lastQuake = this.lastClicked.getProperty(CityProperty.QLAST.name()).toString();
